...
def _gen_feature_nodearray(xi, feature_max=None, issparse=True):
...
def gen_feature_nodearray_from_ctypes_array(xi, feature_max=None, issparse=True):
    ...
    if feature_max:
        raise Exception('feature_max is not supported! value: ' + str(feature_max))
        #assert(isinstance(feature_max, int))
        #values = filter(lambda x: x[0] <= feature_max, values)
    if issparse:
        raise Exception('issparse=True is not supported!')
